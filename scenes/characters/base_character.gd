...
var poison_flask_scene = preload("res://scenes/weapons/poison_flask_weapon.tscn")
var virus_bomb_scene = preload("res://scenes/weapons/virus_bomb_weapon.tscn")
...
func _input(event: InputEvent) -> void:
	...
	if is_multiplayer_authority():
		...
		if event.is_action_released("number_4"):
			_on_weapon_instance(poison_flask_scene)
		if event.is_action_released("number_5"):
			_on_weapon_instance(virus_bomb_scene)
